...
#my $star_path = "/usr/local/bioinfo/STAR/2.7.2b/bin";
...
#
#  Script Name : Rnaseq_paired_end_fastq_to_Vcf_gvcf_jobarray_Total_GATK4.pl
#
...
my $cutadapt = "$cutadapt_path/cutadapt -b AGATCGGAAGAGC -B AGATCGGAAGAGC -O 7 -m 30 -q 20,20 -o $forward_without_ext.cutadapt.fastq -p $reverse_without_ext.cutadapt.fastq $forward $reverse";
print ("cutadapt -b AGATCGGAAGAGC -O 7 -m 30 -q 20,20 -o $forward_without_ext.cutadapt.fastq -p $reverse_without_ext.cutadapt.fastq $forward $reverse\n");
system("$cutadapt");

#Check the reads quality : fastqc
...
#STAR mapping on the reference choosen in argument -r
my $star_mapping = "$star_path/STAR --genomeDir $genome_indexes_dir --readFilesIn $forward_without_ext.cutadapt.fastq $reverse_without_ext.cutadapt.fastq --outSAMunmapped Within --outFileNamePrefix $filenm_root. --outSAMmapqUnique 255 --twopassMode Basic";
...
my $add_rg = "$java_path/java -Xmx4g -jar $picard_path/picard.jar AddOrReplaceReadGroups I=$filenm_root.Aligned.out.sam O=$filenm_root.RG.sorted.bam SO=coordinate RGLB=$cultivar RGPL=illumina RGPU=run RGSM=$condition RGID=$condition";
print ("Adding Read Group and sorting on file $filenm_root.Aligned.out.sam...\n");
system("$add_rg");

#Markduplicates and create index, with the tools picardtools
my $markduplicates = "$java_path/java -Xmx4g -jar $picard_path/picard.jar MarkDuplicates I=$filenm_root.RG.sorted.bam O=$filenm_root.dedupped.RG.sorted.bam CREATE_INDEX=true VALIDATION_STRINGENCY=SILENT M=$filenm_root.output.mectrics";
...
my $split_and_trim = "$gatk_path/gatk --java-options -Xmx4G SplitNCigarReads -R $reference -I $filenm_root.dedupped.RG.sorted.bam -O $filenm_root.split.dedupped.RG.sorted.bam";
print ("Split N CigarReads on file $filenm_root.dedupped.RG.sorted.bam...\n");
system("$split_and_trim");
#=================================================================================================================================
...
my $realigner_target = "$java_path/java -Xmx4g -XX:ParallelGCThreads=1 -jar $gatk3_path/GenomeAnalysisTK.jar -T RealignerTargetCreator --filter_reads_with_N_cigar -R $reference -I $filenm_root.split.dedupped.RG.sorted.bam -o $filenm_root.forIndelRealigner.intervals";
...
my $indelrealigner = "$java_path/java -Xmx4g -XX:ParallelGCThreads=1 -jar $gatk3_path/GenomeAnalysisTK.jar -T IndelRealigner --filter_reads_with_N_cigar -R $reference -I $filenm_root.split.dedupped.RG.sorted.bam -targetIntervals $filenm_root.forIndelRealigner.intervals -o $filenm_root.indelrealigned.split.dedupped.RG.sorted.bam";
...
my $haplotype_caller = "$gatk_path/gatk --java-options -Xmx5G HaplotypeCaller -R $reference -I $filenm_root.indelrealigned.split.dedupped.RG.sorted.bam -ploidy 2 --dont-use-soft-clipped-bases -O $filenm_root.indelrealigned.split.dedupped.RG.sorted.HC.vcf";
print ("Call SNPs with HaplotypeCaller on file $filenm_root.indelrealigned.RG.sorted.bam...\n");
system("$haplotype_caller");


#HaplotypeCaller (for BaseRecalibrator)  ###### Pour une ancienne version GATK3.4
...
#my $variant_annotation_First = "$gatk_path/gatk --java-options -Xmx4G VariantAnnotator -R $reference -V $filenm_root.indelrealigned.split.dedupped.RG.sorted.HC.vcf -I $filenm_root.indelrealigned.split.dedupped.RG.sorted.bam -A MappingQualityZero -O $filenm_root.indelrealigned.split.dedupped.RG.sorted.HC.ann.vcf";
...
##### Second HaplotypeCaller on recalibrated Bam.
#HaplotypeCaller GATK4 (pour BaseRecalibrator)
my $haplotype_caller_vcf = "$gatk_path/gatk --java-options -Xmx5G HaplotypeCaller -R $reference -I $filenm_root.indelrealigned.split.dedupped.RG.sorted.recalibrated.bam -ploidy 2 -stand-call-conf 20.0 --dont-use-soft-clipped-bases -O $filenm_root.indelrealigned.split.dedupped.RG.sorted.recalibrated.HC.vcf";
print ("Call SNPs with HaplotypeCaller on file $filenm_root.indelrealigned.split.dedupped.RG.sorted.bam ...\n");
