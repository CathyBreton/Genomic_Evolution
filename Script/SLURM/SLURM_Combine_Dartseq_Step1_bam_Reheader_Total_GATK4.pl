...
#print "@bamfile_rep \n";    
my %bam_by_pre=();## Faire un aray vide 
...
#=================================================================================================================================
   ...
   #if ($bam =~ m/^(\d+)_(.*)\.bam/) ## regEx to find a number example 923
   if ($bam =~ m/^(.\d.)_(.*)\.bam/) ## regEx to find everything with a letter and a number example H20 or 923
   
   #if ($bam =~ m/^(.*)_(.*)\.bam/) ## regEx to find H20_5   or 923_5
      {
        my $prefix = $1;
        my $description = $2;
        print "$bam --> $prefix\n";
...
#=================================================================================================================================
         ...
         print "#--------------------------------#\n";
         print "Step 1 Add read group \n";
         print "#--------------------------------#\n";

         
         my $add_rg_rep = "java -Xmx50G -jar $picard_path_2_72/picard.jar AddOrReplaceReadGroups"
         . ' ' . "I=$bam"
         . ' ' . "O=$name_file.reheader.bam"
         . ' ' . "RGLB=$cultivar"
         . ' ' . "RGPL=illumina"
         . ' ' . "RGPU=BforBB"
         . ' ' . "RGSM=$prefix"
         . ' ' . "\n";
         print ("Adding Read Group and sorting on file $bam...\n");
         print "  $add_rg_rep\n"; #+debug
         #system($add_rg_rep) and die "cannot run sambamba : $!";
         if (system("$add_rg_rep"))
         {   
                warn "Failed to execute: $!\n";  # system() returned an error code, execution failed.
         } 
