...
my $picard_path     = "/home/bretonc/work_agap_id-bin/img/picard-tools/2.24.0/picard-tools.2.24.0.img";
...
{
    print "Warning: unknown version or no version specified! we will use version 4.1 \n\n";
    $gatk_path = $gatk_version{'4.1'};  
}
...
if (-e "$directory/$output_prefix.gvcf")
{
    ...
    {
        my $GenotypeGVCFs_cmd = "$gatk_path/gatk --java-options -Xmx5g GenotypeGVCFs --allow-old-rms-mapping-quality-annotation-data -R $reference -V $directory/$output_prefix.gvcf -O $directory/$output_prefix.Genotype.vcf";
